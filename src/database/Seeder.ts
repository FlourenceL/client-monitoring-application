...
export const seedDatabase = async () => {
  try {
    ...
    // Seed Locations
    const locations = await databaseService.query(`SELECT * FROM ${MstLocation}`);
    if (locations.length === 0) {
      const locationData = ["Main Office", "Branch 1", "Branch 2"];
    ...
    if (plans.length === 0) {
      const planData = [
        { name: "Basic Plan", amount: 500, isActive: 1 },
        { name: "Premium Plan", amount: 1000, isActive: 1 },
      ];
    ...
    if (statuses.length === 0) {
        ...
        for(const status of statusData){
             await databaseService.run(`INSERT INTO ${MstStatus} (Status) VALUES (?)`, [status]);
        }
     ...
     const clients = await databaseService.query(`SELECT * FROM ${MstClient}`);
      if (clients.length === 0) {
        ...
        for (const client of clientData) {
            await databaseService.run(`INSERT INTO ${MstClient} (Client, ContactInfo, DateInstalled, PlanId, IsActive) VALUES (?, ?, ?, ?, ?)`,
                [client.client, client.contactInfo, client.dateInstalled, client.planId, client.isActive]
            );
        }
